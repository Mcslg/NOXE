...
import { ArrowRight, Sparkles, GraduationCap, MessageSquareCode, BrainCircuit, FileText, Key, Check, ExternalLink } from 'lucide-react';
...
export const TopicInput: React.FC = () => {
  const { startSession, apiKey, setApiKey, isLoading } = useStudy();
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 md:py-14 space-y-8">
      ...
      <div className="text-center space-y-3">
        ...
        <p className="text-slate-600 max-w-xl mx-auto text-xs md:text-sm leading-relaxed">
          輸入你想複習或掌握的概念。AI 將化身為初學者學生向你提問與追問，
          透過「白話輸出 $\rightarrow$ 查漏補缺 $\rightarrow$ 弱點重測」，最後自動產出個人專屬結構化筆記。
        </p>
      </div>

      {/* 若尚未填寫 API Key，在最顯眼處展示設定區塊 */}
      {!apiKey && (
        <div className="bg-amber-50/90 border-2 border-amber-300/80 rounded-2xl p-5 md:p-6 shadow-sm space-y-4 animate-fadeIn">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-sm md:text-base font-bold text-amber-950">
                步驟 1：請先設定 Google Gemini API Key
              </h3>
              <p className="text-xs text-amber-800 mt-1 leading-relaxed">
                金鑰僅保存在您的瀏覽器本地（LocalStorage），完全不經過第三方伺服器，安全有保障。
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSaveKey} className="flex flex-col sm:flex-row gap-2.5">
            <input
              ...
              placeholder="在此貼上您的 Gemini API Key (例如: AIzaSy...)"
              className="flex-1 px-4 py-2.5 text-sm bg-white border border-amber-300 rounded-xl outline-none focus:ring-2 focus:ring-amber-500 text-slate-800"
              required
            />
            <button
              type="submit"
              disabled={!tempApiKey.trim()}
              className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 disabled:bg-amber-300 text-white text-xs font-bold rounded-xl shadow-sm transition-all flex items-center justify-center gap-1.5 shrink-0"
            >
              {isKeySaved ? <Check className="w-4 h-4" /> : null}
              {isKeySaved ? '已儲存金鑰' : '儲存 API Key'}
            </button>
          </form>

          <div className="text-xs text-amber-700 flex items-center gap-1.5 font-medium pt-1">
            ...
            >
              Google AI Studio 獲取免費金鑰 <ExternalLink className="w-3.5 h-3.5" />
            </a>
          </div>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="relative">
          <div className="relative flex items-center shadow-lg shadow-blue-500/5 rounded-2xl overflow-hidden border-2 border-blue-600/30 focus-within:border-blue-600 transition-colors bg-white">
            <input
              ...
              placeholder="步驟 2：請輸入想複習的主題（例如：TCP 三向交握、JavaScript 閉包...）"
              disabled={isLoading}
              className="w-full pl-5 pr-32 py-4 text-sm md:text-base outline-none text-slate-800 placeholder:text-slate-400"
            />
            <button
              type="submit"
              disabled={isLoading || !topic.trim()}
              className="absolute right-2 top-2 bottom-2 px-5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white font-medium rounded-xl flex items-center gap-1.5 shadow-sm transition-all text-sm"
            >
              <span>開始教學</span>
